...
public:
  ...
  inline void TestInitial() {
    ...
    for (int count = 0; count < example_num; count++) {
      const Example& example = examples[count];
      cost += _modelparams.loss.loss(&_builders[count]._hidden[0], example.m_label, _eval, example_num);
    }
    ...
    _builders[0].forward(feature);
		...
		_modelparams.loss.predict(&_builders[0]._hidden[0], result);
	}
    ...
    dtype cost = _modelparams.loss.cost(&_builders[0]._hidden[0], example.m_label, 1);
