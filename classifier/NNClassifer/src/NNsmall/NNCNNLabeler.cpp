...
void Classifier::predict(const vector<Example>& examples, vector<string>& outputs) {
	vector<int> labelIdxs;
	m_driver.predict(examples, labelIdxs);
	int label_size = labelIdxs.size();
	assert(label_size == examples.size());
	for (int idx = 0; idx < label_size; idx++) {
		string output;
		output = m_driver._modelparams.labelAlpha.from_id(labelIdxs[idx], unknownkey);
		if (output == nullkey) {
