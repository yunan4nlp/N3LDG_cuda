...
__global__ void Fadd_inplace_kernel(dtype** x, dtype*** y, int n, int dim0, int size) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if(index < size) {
		int x_idx = index / dim0;
		int x_idy = index % dim0;
		x[x_idx][x_idy] = 0;
		int y_idx = x_idx;
		int y_idz = x_idy;
		for(int i = 0; i < n; i++) {
			x[x_idx][x_idy] += y[y_idx][i][y_idz];
		}
	}
}

void Fadd_inplace_impl(dtype** x, dtype*** y, int count, int n, int dim0) {
	int size = count * dim0;
	Fadd_inplace_kernel<<<(size + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(x, y, n, dim0, size);
