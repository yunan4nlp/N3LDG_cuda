...
class ArrayPtr {
	...
	ArrayPtr(const vector<LDG::PTensor>& vec) {
		...
		dtype **v_data = static_cast<dtype **>(ptr_.get());
	...
	public:
		...
		*/
		...
		void set(vector<LDG::PTensor> &vec_t, dtype val) {
			...
			std::shared_ptr<void> tmp_dims_ptrs = mem_pool->allocate(mem_size);
			int *dims_ptr = static_cast<int *>(tmp_dims_ptrs.get());
			cudaMemcpy(dims_ptr, (int*)vec_dims.data(), mem_size, cudaMemcpyHostToDevice);

			vector<const dtype*> vec_ptr_t(n);
			for(int i = 0; i < n; i++) {
				vec_ptr_t[i] = (CDATA(*vec_t[i]));
			}
			mem_size = sizeof(dtype*) * n;
			std::shared_ptr<void> t_ptrs = mem_pool->allocate(mem_size); 
			dtype **v_data_t = static_cast<dtype **>(t_ptrs.get());
			cudaMemcpy(v_data_t, (dtype**)vec_ptr_t.data(), mem_size, cudaMemcpyHostToDevice);

			set_impl(v_data_t, dims_ptr, n, max_dim, val);
		}
		...
		void set_cols(LDG::Tensor &t, const vector<int>& cols, const vector<dtype>& vals){
			...
			std::shared_ptr<void> gpu_ptrs = mem_pool->allocate(mem_size); 
			int *gpu_cols = static_cast<int *>(gpu_ptrs.get());
			cudaMemcpy(gpu_cols, cols.data(), mem_size, cudaMemcpyHostToDevice);

			
			mem_size = sizeof(dtype) * vals.size();
			std::shared_ptr<void> vals_ptr = mem_pool->allocate(mem_size); 
			dtype *gpu_vals = static_cast<dtype *>(vals_ptr.get());
			cudaMemcpy(gpu_vals, vals.data(), mem_size, cudaMemcpyHostToDevice);

			set_cols_impl(MDATA(t), dim0, gpu_cols, col_num, gpu_vals);
		}
		...
		void FLookup(const LDG::Tensor& x, int* cols, int col_num, vector<LDG::PTensor>& vec_r) {
			...
			int n = col_num;
			int mem_size = sizeof(int) * n;
			std::shared_ptr<void> cols_ptrs = mem_pool->allocate(mem_size); 
			...
			cudaMemcpy(v_data_r, (dtype**)vec_ptr_r.data(), sizeof(dtype*) * col_num, cudaMemcpyHostToDevice);
		...
		void DLookup(LDG::Tensor& gx, int* cols, int col_num, vector<LDG::PTensor>& vec_loss) {
			...
			vector<const dtype*> vec_ptr_loss(n);
			for(int i = 0; i < n; i++) {
				vec_ptr_loss[i] = (MDATA(*vec_loss[i]));
			}
			std::shared_ptr<void> loss_ptrs = mem_pool->allocate(sizeof(dtype*) * col_num); 
			dtype **v_data_loss = static_cast<dtype **>(loss_ptrs.get());
			cudaMemcpy(v_data_loss, (dtype**)vec_ptr_loss.data(), sizeof(dtype*) * col_num, cudaMemcpyHostToDevice);
		...
		void Fadd_inplace(vector<LDG::PTensor>& vec_x, const vector<vector<LDG::PTensor> >& vec_vec_y) {
			int count = vec_vec_y.size();
			int n = vec_vec_y[0].size();
			assert(count == vec_x.size());
			
			for(int idx = 1; idx < count; idx++)
				assert(vec_vec_y[idx].size() == n);
			int x_dim = vec_x[0]->shape()[0];
			for(int idx = 0; idx < count; idx++) {
				assert(vec_x[idx]->shape()[0] == x_dim);
				for(int idy = 0; idy < n; idy++) {
					assert(x_dim == vec_vec_y[idx][idy]->shape()[0]);
				}
			}

			int mem_size = sizeof(dtype**) * count;
			std::shared_ptr<void> y_ptrs = mem_pool->allocate(mem_size); 
			dtype ***v_v_data_y = static_cast<dtype ***>(y_ptrs.get());

			vector<dtype**> vec_vec_ptr_y(count);
			vector<std::shared_ptr<void> > vec_tmp_ptr(count); 
			for (int idx = 0; idx < count; idx++) {
				const vector<LDG::PTensor> vec_y = vec_vec_y[idx];
				...
				dtype **v_data_y = static_cast<dtype **>(vec_tmp_ptr[idx].get());
				cudaMemcpy(v_data_y, (dtype**)vec_ptr_y.data(), mem_piece_size, cudaMemcpyHostToDevice);

				vec_vec_ptr_y[idx] = v_data_y;
				//concat_impl(vec_vec_ptr_x[idx] , MDATA(*vec_r[idx]), src_dim, src_dim * n);
			}
			cudaMemcpy(v_v_data_y, (dtype***)vec_vec_ptr_y.data(), mem_size, cudaMemcpyHostToDevice);


			/*
			mem_size = sizeof(dtype*) * count;
			vector<const dtype*> vec_ptr_x(count);
			...
			*/
			...
			dtype **v_data_x = (dtype **)x_ptr.get_ptr();
			
			Fadd_inplace_impl(v_data_x, v_v_data_y, count, n, x_dim);
		}
...
/*
		...
		*/
		...
		void concat(const vector<LDG::PTensor>& vec_x, LDG::Tensor& r) {
			...
			for(int idx = 1; idx < max_size; idx++) {
				if(!xshape.has_same_dims(vec_x[idx]->shape())) {
			...
			if(all_same_shape) {
				...
				dtype **v_data_x = static_cast<dtype **>(x_ptrs.get());
				cudaMemcpy(v_data_x, (dtype**)vec_ptr_x.data(), mem_size, cudaMemcpyHostToDevice);
				concat_impl(v_data_x, MDATA(r), src_dim, dst_dim);
			} else {
				int offset = 0;
		...
		void unconcat(const LDG::Tensor& r, vector<LDG::PTensor>& vec_x) {
			...
			if(all_same_shape) {
				...
				int n = vec_x.size();
				int mem_size = sizeof(dtype*) * n;
				vector<dtype*> vec_ptr_x(n);
				for(int i = 0; i < n; i++) {
					vec_ptr_x[i] = (MDATA(*vec_x[i]));
				}
				...
				unconcat_impl(CDATA(r), v_data_x, src_dim, dst_dim);
			} else {
				int offset = 0;
				for(int idx = 0; idx < max_size; idx++) {
					int dim_0 = vec_x[idx]->shape()[0];
		...
		void concat(const vector<vector<LDG::PTensor> >& vec_vec_x, vector<LDG::PTensor>& vec_r) {
			...
			std::shared_ptr<void> x_ptrs = mem_pool->allocate(mem_size); 
			dtype ***v_v_data_x = static_cast<dtype ***>(x_ptrs.get());
			
			vector<dtype**> vec_vec_ptr_x(count);
			vector<std::shared_ptr<void> > vec_tmp_ptr(count); 
			for (int idx = 0; idx < count; idx++) {
				const vector<LDG::PTensor> vec_x = vec_vec_x[idx];
				int mem_piece_size = sizeof(dtype*) * n;
				vector<const dtype*> vec_ptr_x(n);
				for(int i = 0; i < n; i++) {
					vec_ptr_x[i] = (CDATA(*vec_x[i]));
				...
				vec_tmp_ptr[idx] = mem_pool->allocate(mem_piece_size); 
				dtype **v_data_x = static_cast<dtype **>(vec_tmp_ptr[idx].get());
				cudaMemcpy(v_data_x, (dtype**)vec_ptr_x.data(), mem_piece_size, cudaMemcpyHostToDevice);

				vec_vec_ptr_x[idx] = v_data_x;
				//concat_impl(vec_vec_ptr_x[idx] , MDATA(*vec_r[idx]), src_dim, src_dim * n);
			}

			cudaMemcpy(v_v_data_x, (dtype***)vec_vec_ptr_x.data(), mem_size, cudaMemcpyHostToDevice);

			mem_size = sizeof(dtype*) * count;
			vector<const dtype*> vec_ptr_r(count);
			for(int i = 0; i < count; i++) {
				vec_ptr_r[i] = (CDATA(*vec_r[i]));
			}
			std::shared_ptr<void> r_ptrs = mem_pool->allocate(mem_size); 
			dtype **v_data_r = static_cast<dtype **>(r_ptrs.get());
			cudaMemcpy(v_data_r, (dtype**)vec_ptr_r.data(), mem_size, cudaMemcpyHostToDevice);

			concat_impl(v_v_data_x, count, n, src_dim, src_size, v_data_r);	
		}

		void unconcat(const vector<LDG::PTensor>& vec_r, vector<vector<LDG::PTensor> >& vec_vec_x) {
			int count = vec_vec_x.size();
			int n = vec_vec_x[0].size();
			assert(count == vec_r.size());
			...
			unconcat_impl(v_data_r, count, n, src_dim, src_size, v_v_data_x);	
		}
		...
		void Fconcat(const vector<vector<LDG::PTensor> >& vec_vec_x, vector<LDG::PTensor>& vec_r) {
			...
			int mem_size = sizeof(int) * n;
			std::shared_ptr<void> tmp_offset_ptrs = mem_pool->allocate(mem_size); 
			int *offset_ptr = static_cast<int *>(tmp_offset_ptrs.get());
			...
			mem_size = sizeof(dtype**) * count;
			std::shared_ptr<void> x_ptrs = mem_pool->allocate(mem_size); 
			dtype ***src = static_cast<dtype ***>(x_ptrs.get());
			...
			cudaMemcpy(src, (dtype***)vec_vec_ptr_x.data(), mem_size, cudaMemcpyHostToDevice);
			...
			std::shared_ptr<void> r_ptrs = mem_pool->allocate(mem_size); 
			dtype **dst = static_cast<dtype **>(r_ptrs.get());
			cudaMemcpy(dst, (dtype**)vec_ptr_r.data(), mem_size, cudaMemcpyHostToDevice);

			int dst_dim = vec_r[0]->shape()[0];

			Fconcat_impl(src, count, n, offset_ptr, dims_ptr, max_dim, dst);
		}
		...
		void Dconcat(vector<vector<LDG::PTensor> >& vec_vec_in_loss, const vector<LDG::PTensor>& vec_loss) {
			...
			mem_size = sizeof(dtype*) * count;
			vector<const dtype*> vec_ptr_loss(count);
			for(int i = 0; i < count; i++) {
				vec_ptr_loss[i] = (CDATA(*vec_loss[i]));
			}
			std::shared_ptr<void> loss_ptrs = mem_pool->allocate(mem_size); 
			dtype **loss = static_cast<dtype **>(loss_ptrs.get());
			cudaMemcpy(loss, (dtype**)vec_ptr_loss.data(), mem_size, cudaMemcpyHostToDevice);


			mem_size = sizeof(dtype**) * count;
			std::shared_ptr<void> in_loss_ptrs = mem_pool->allocate(mem_size); 
			dtype ***in_loss = static_cast<dtype ***>(in_loss_ptrs.get());
			vector<dtype**> vec_vec_ptr_in_loss(count);
			vector<std::shared_ptr<void> > vec_tmp_ptr(count); 
			for (int idx = 0; idx < count; idx++) {
				const vector<LDG::PTensor> vec_in_loss = vec_vec_in_loss[idx];
				int mem_piece_size = sizeof(dtype*) * n;
				vector<const dtype*> vec_ptr_in_loss(n);
				for(int i = 0; i < n; i++) {
					vec_ptr_in_loss[i] = (CDATA(*vec_in_loss[i]));
				}
				vec_tmp_ptr[idx] = mem_pool->allocate(mem_piece_size); 
				dtype **v_data_in_loss = static_cast<dtype **>(vec_tmp_ptr[idx].get());
				cudaMemcpy(v_data_in_loss, (dtype**)vec_ptr_in_loss.data(), mem_piece_size, cudaMemcpyHostToDevice);
				vec_vec_ptr_in_loss[idx] = v_data_in_loss;
				//concat_impl(vec_vec_ptr_x[idx] , MDATA(*vec_r[idx]), src_dim, src_dim * n);
			}
			cudaMemcpy(in_loss, (dtype***)vec_vec_ptr_in_loss.data(), mem_size, cudaMemcpyHostToDevice);

			Dconcat_impl(loss, count, n, offset_ptr, dims_ptr, max_dim, in_loss);
		}
		/*
		...
		*/
		...
		void FSumPooling(const vector<vector<LDG::PTensor> >& vec_vec_x, vector<LDG::PTensor>& vec_y) {
		}
		...
		vector<dtype> to_vector(const LDG::Tensor& x, const vector<int>& cols) {
			...
			get_cols_impl(CDATA(x), MDATA(r_tensor), xdim0, xdim1, r_size, gpu_cols, cols.size());
