...
__global__ void concat_kernel(dtype ***src, int count, int n, int *offset_ptr, int *dims, int y_dim,  int max_dim, int dst_size, dtype *dst) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if(index < dst_size) {
		int dst_idx = index / (n * max_dim);
		...
		if(src_idz < dims[src_idy]) {
			...
			int dst_idy = offset_ptr[src_idy] + src_idz;
			dst[dst_idx * y_dim + dst_idy] = src[src_idx][src_idy][src_idz];
			//printf("%d, %d, %d, %d\n", index, src_idx, src_idy, src_idz);
		}
...
void concat_impl(dtype ***src, int count, int n, int *offset_ptr, int *dims, int y_dim, int max_dim, dtype *dst) {
	int dst_size = count * n * max_dim;
	concat_kernel<<<(dst_size + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(src, count, n, offset_ptr, dims, y_dim, max_dim, dst_size, dst);
	//cout << "Error: " << cudaGetErrorString(cudaGetLastError()) << endl;
	//cudaDeviceSynchronize();
}
