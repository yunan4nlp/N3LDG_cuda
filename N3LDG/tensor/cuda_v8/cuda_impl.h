	...
	public:
		...
		*/
		...
		void set(vector<LDG::PTensor> &vec_t, dtype val) {
			...
			cudaMemcpyAsync(dims_ptr, (int*)vec_dims.data(), mem_size, cudaMemcpyHostToDevice, 0);

			ArrayPtr t_ptr(vec_t);
			dtype **t = (dtype**)t_ptr.get_ptr();

			set_impl(t, dims_ptr, n, max_dim, val);
		}
		...
		void set_cols(LDG::Tensor &t, const vector<int>& cols, const vector<dtype>& vals){
			...
			cudaMemcpyAsync(gpu_cols, cols.data(), mem_size, cudaMemcpyHostToDevice, 0);
			...
			cudaMemcpyAsync(gpu_vals, vals.data(), mem_size, cudaMemcpyHostToDevice, 0);
		...
		void get_cols(const LDG::Tensor& x, int* cols, int col_num, LDG::Tensor& r) {
			...
			cudaMemcpyAsync(gpu_cols, cols, memsize, cudaMemcpyHostToDevice, 0);
		...
		void FLookup(const LDG::Tensor& x, int* cols, int col_num, vector<LDG::PTensor>& vec_r) {
			...
			cudaMemcpyAsync(gpu_cols, cols, mem_size, cudaMemcpyHostToDevice, 0);
			...
			if(xdim0 == rdim0) {
				FLookup_impl(CDATA(x), r, xdim0, xdim1, r_size, gpu_cols, col_num);
			} else
		...
		void DLookup(LDG::Tensor& gx, int* cols, int col_num, vector<LDG::PTensor>& vec_loss) {
			...
			if(gxdim0 == ldim0) {
				DLookup_impl(MDATA(gx), loss, gxdim0, gxdim1, l_size, gpu_cols, col_num);
			} else
				cout << "get col dims are not matched" << endl;
		}
		...
		void Fadd_inplace(LDG::Tensor& x, const vector<LDG::PTensor>& vec_y) {
			...
			Fadd_inplace_impl(MDATA(x), y, n, x_size);
		}
		...
		void Fmatmul(const LDG::Tensor &x, const vector<LDG::PTensor> &vec_y, vector<LDG::PTensor> &vec_r, bool tx = false, bool ty = false) {
			...
			for (int idx = 0; idx < size_y; idx++) {
				...
				int n = ty ? ptr_y->shape()[0] : ptr_y->shape()[1];
				malloc(*ptr_r, Shape({m, n}));
			}
		...
		void Fmatmul(const LDG::Tensor &x, const vector<vector<LDG::PTensor> > &vec_vec_y, vector<LDG::PTensor> &vec_r, bool tx = false, bool ty = false) {
			int y_size = vec_vec_y.size();
			int y_dim0 = 0;
			int n = vec_vec_y[0].size();
			for(int idx = 0; idx < n; idx++) {
				y_dim0 += vec_vec_y[0][idx]->shape()[0];
			}

			int x_dim0 = x.shape()[0];
			assert(vec_r.size() == y_size);
			for(int idx = 0; idx < y_size; idx++) {
				malloc(*vec_r[idx], Shape({x_dim0, 1}));
			}
			LDG::Tensor y;
			concat(vec_vec_y, y);
...
/*
		...
		*/
		...
		void concat(const vector<vector<LDG::PTensor> >& vec_vec_x, LDG::Tensor& r) {
			...
			cudaMemcpyAsync(offset_ptr, (int*)dim_offset.data(), mem_size, cudaMemcpyHostToDevice, 0);

			std::shared_ptr<void> tmp_dims_ptrs = MEM_POOL->allocate(mem_size); 
			int *dims_ptr = static_cast<int *>(tmp_dims_ptrs.get());
			//cudaMemcpy(dims_ptr, (int*)dims.data(), mem_size, cudaMemcpyHostToDevice);
			cudaMemcpyAsync(dims_ptr, (int*)dims.data(), mem_size, cudaMemcpyHostToDevice, 0);

			ArrayPtr src_ptr(vec_vec_x);
			dtype ***src = (dtype ***)src_ptr.get_ptr();

			concat_impl(src, count, n, offset_ptr, dims_ptr, offset, max_dim, MDATA(r));
		}
		...
		void concat(const vector<vector<LDG::PTensor> >& vec_vec_x, vector<LDG::PTensor>& vec_r) {
			...
			int max_dim = -1;
			vector<int> dims(count);
			for(int idx = 0; idx < count; idx++) {
				int dim = vec_vec_x[idx][0]->shape()[0];
				malloc(*vec_r[idx], Shape({dim, n}));
				if(max_dim < dim)
					max_dim = dim;
				dims[idx] = dim;
			}
			...
			concat_impl(src, dims_ptr, max_dim, count, n, dst);
		}
		...
		void unconcat(const vector<LDG::PTensor>& vec_r, vector<vector<LDG::PTensor> >& vec_vec_x) {
			...
			ArrayPtr src_ptr(vec_r);
			dtype **src = (dtype **)src_ptr.get_ptr();

			ArrayPtr dst_ptr(vec_vec_x);
			dtype ***dst = (dtype ***)src_ptr.get_ptr();

			unconcat_impl(src, count, n, src_dim, src_size, dst);	
		}
		...
		*/
		...
		void FMaxPooling(const vector<LDG::PTensor>& vec_x, LDG::Tensor& y, int* index) {
			...
			Fmaxpooling_impl(x, MDATA(y), n, r, s, gpu_index_data);
			//cudaMemcpy(index, gpu_index_data, sizeof(int) * r, cudaMemcpyDeviceToHost);
			cudaMemcpyAsync(index, gpu_index_data, sizeof(int) * r, cudaMemcpyDeviceToHost, 0);
		}
		...
		void DMaxPooling(const LDG::Tensor& gy, vector<LDG::PTensor>& vec_gx, int* index) {
			...
			cudaMemcpyAsync(gpu_index_data, index, sizeof(int) * dim0, cudaMemcpyHostToDevice, 0);
			Dmaxpooling_impl(CDATA(gy), gx, gpu_index_data, dim0);
		}
		...
		void FMinPooling(const vector<LDG::PTensor>& vec_x, LDG::Tensor &y, int* index) {
			...
			Fminpooling_impl(x, MDATA(y), n, r, s, gpu_index_data);
			//cudaMemcpy(index, gpu_index_data, sizeof(int) * r, cudaMemcpyDeviceToHost);
			cudaMemcpyAsync(index, gpu_index_data, sizeof(int) * r, cudaMemcpyDeviceToHost, 0);
		}
		...
		void DMinPooling(const LDG::Tensor& gy, vector<LDG::PTensor>& vec_gx, int* index) {
			...
			//cudaMemcpy(gpu_index_data, index, sizeof(int) * dim0, cudaMemcpyHostToDevice);
			cudaMemcpyAsync(gpu_index_data, index, sizeof(int) * dim0, cudaMemcpyHostToDevice, 0);
			Dminpooling_impl(CDATA(gy), gx, gpu_index_data, dim0);
		}
