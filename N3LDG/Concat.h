...
class ConcatNode : public Node {
  ...
  public:
    ...
    void backward() {
        int nSize = ins.size();
		LDG::Tensor array_ins_loss[nSize];
		vector<LDG::PTensor> vec_ins_loss;
        for (int i = 0; i < nSize; ++i) {
			device.init(array_ins_loss[i], ins[i]->loss.shape());
			vec_ins_loss.push_back(&array_ins_loss[i]);
        }
		device.unconcat(loss, vec_ins_loss);

        for (int i = 0; i < nSize; ++i) {
			device.Fadd(ins[i]->loss, *vec_ins_loss[i], ins[i]->loss);
		}
