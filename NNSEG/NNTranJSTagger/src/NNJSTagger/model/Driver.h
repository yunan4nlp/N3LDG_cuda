...
class Driver {
  ...
  private:
    dtype loss_google(GreedyGraphBuilder& builder, int num) {
		...
		device.to_cpu(pCurNode->val, cpu_cur_val);

		cpu_best_val.device_type = CPU;
		cpu_best_val.shape_ = pBestNode->val.shape();
		cpu_best_val.v = new dtype[pBestNode->val.shape().size()];
		device.to_cpu(pBestNode->val, cpu_best_val);


                if (cpu_cur_val.v[0] > cpu_best_val.v[0]) {
                    pBestNode = pCurNode;
		    max = cpu_best_val.v[0];
                }
	    ...
	    device.set(pGoldNode->loss, data, 1);
		...
		device.to_cpu(pCurNode->val, cpu_cur_val);
                ...
                scores[idx] = exp(cpu_cur_val.v[0] - max);
                sum += scores[idx];
            }

            for (int idx = 0; idx < curcount; idx++) {
                pCurNode = builder.outputs[step][idx].in;
                device.Fadd_scalar(pCurNode->loss, scores[idx] / (sum * num),  pCurNode->loss);
    ...
    dtype loss_google(BeamGraphBuilder& builder, int num) {
		...
		device.to_cpu(pBestNode->val, cpu_best_val);
                ...
                if (pBestNode == NULL || cpu_cur_val.v[0] > cpu_best_val.v[0]) {
                    pBestNode = pCurNode;
					max = cpu_best_val.v[0];
                }
                if (builder.outputs[step][idx].bGold) {
