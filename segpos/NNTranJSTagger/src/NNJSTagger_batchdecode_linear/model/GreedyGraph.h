...
public:
	...
	inline void decode_apply_action(Graph* pcg, const std::vector<std::string>* pCharacters, const vector<CAction>* goldAC = NULL) {
		...
		if (!is_finish) {
			...
			//if (pcg->train) pGenerator->_nextscores.score.val.vec() += pOpts->delta;

			for (int idy = 0; idy < actions.size(); ++idy) {
				int action_id = pOpts->action_alpha.from_string(actions[idy].str(pOpts));
			...
			if (pcg->train) {
				...
				for (int idx = 0; idx < offset; idx++) {
					if (beam[idx].bGold) {
						...
						pGenerator->move(&(states[next_step]), action);
						states[next_step]._bGold = beam[idx].bGold;
						//states[next_step]._score = &(pGenerator->_nextscores.outputs[beam[idx].position]);
						find_next = true;
					}
				}
				...
				pGenerator->move(&(states[next_step]), action);
				states[next_step]._bGold = beam[0].bGold;
				//states[next_step]._score = &(pGenerator->_nextscores.outputs[beam[0].position]);
			}
			pGenerator = &(states[next_step]);
			if (states[next_step].IsTerminated()) {
				is_finish = true;
			}
			step++;
		}
