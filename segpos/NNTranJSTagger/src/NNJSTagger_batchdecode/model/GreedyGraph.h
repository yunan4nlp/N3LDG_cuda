			...
			if (!is_finish) {
				...
				for (int idy = 0; idy < actions.size(); ++idy) {
					...
					if (pGenerator->_bGold && actions[idy] == answer) {
						...
						output.bGold = false;
						if (pcg->train) {
							DEV->Fadd_scalar_inplace(pGenerator->_nextscores.outputs[idy].val, pOpts->delta);
						}
					}
					vector<dtype> cpu_score = DEV->to_vector(pGenerator->_nextscores.outputs[idy].val);

					scored_action.score = cpu_score[0];
				...
				for (int idy = 0; idy < actions.size(); ++idy) {
					...
					if (pGenerator->_bGold && actions[idy] == answer) {
						...
						if (pcg->train) {
							...
							//DEV->Fadd_scalar(pGenerator->_nextscores.outputs[idy].val, pOpts->delta,
							//pGenerator->_nextscores.outputs[idy].val);
							//pGenerator->_nextscores.outputs[idy].val[0] += pOpts->delta
						};
					}
					/*
					   LDG::Tensor cpu_score;
					   cpu_score.device_type = CPU;
					   cpu_score.shape_ = pGenerator->_nextscores.outputs[idy].val.shape();
					   cpu_score.v = new dtype[pGenerator->_nextscores.outputs[idy].val.shape().size()];
					   device.to_cpu(pGenerator->_nextscores.outputs[idy].val, cpu_score);
					 */
					vector<dtype> cpu_score = DEV->to_vector(pGenerator->_nextscores.outputs[idy].val);
