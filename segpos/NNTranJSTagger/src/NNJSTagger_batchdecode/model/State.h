...
class CStateItem {
  ...
  public:
    ...
    inline std::string str(HyperParams* opt) const {
		...
		vector<dtype> cpu_score_val = DEV->to_vector(_score->val);
		/*
		LDG::Tensor cpu_score_val;
		cpu_score_val.shape_ = _score->val.shape();
		cpu_score_val.device_type = CPU;
		device.to_cpu(_score->val, cpu_score_val);
		*/
        curoutstr << "score: " << cpu_score_val[0] << " ";
